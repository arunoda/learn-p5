class Mover {
    ...
    checkEdges() {
        ...
        if (this.position.y > height - radius) {
            ...
            this.velocity.y *= -0.9;
            // Add friction when we touch with the ground
            // Usually, friction is a force which is in the inverse direction of the velocity
            // For right now, we will use a fixed magnitude, but we need to calculate 
            // if using the normal force with the surface
            const frictionDirection = this.velocity.copy().normalize().mult(-1);
            const frictionMag = 0.5;
            this.addForce(frictionDirection.mult(frictionMag));
